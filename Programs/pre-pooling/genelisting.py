...
#
# Code to make listing of gene loci in a given genome. Output is input for egeneindex.py
# ----------------------------------------------------------------------------------------------- #

import sys
sys.path.insert(0, './') # need to do this so that imports run in command line
from utilities.utils import ensure_dir, ImportGenBankSequence
...
# ----------------------------------------------------------------------------------------------- #
# Parse cds and gene sequences from the genome
cdsFeatureArray = []
geneFeatureArray = []
...
# ----------------------------------------------------------------------------------------------- #
# Differentiate between essential and non essential genes
...
for feature in cdsFeatureArray:
    ...
    if 'locus_tag' in tagDict: # tag allows us to keep trace of the genes we're looking at
    ...
    elif 'db_xref' in tagDict: # backup plan in case our genome does not have a locus_tag
    ...
    # if both locus_tag and db_xref don't exist in your genome file, can create another way of tagging genes by adding
    # another elif statement (also true below)
...
for feature in geneFeatureArray:
    ...
    if 'locus_tag' in tagDict:
        ...
        featureName = feature.featureName
        essentiality = 'Dispensable'
        geneLocusAndFeatureNameArray.append(locusTag)
        geneLocusNameArray.append(locusTag)
# ----------------------------------------------------------------------------------------------- #
