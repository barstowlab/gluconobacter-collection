...
class Feature3:
    ...
    def updateName(self):
        ...
        elif 'product' in featureTagDictKeys:
            geneName = self.tagDict['product'][0]
        elif 'label' in featureTagDictKeys:
            geneName = self.tagDict['label'][0]
        elif 'locus_tag' in featureTagDictKeys:
            geneName = self.tagDict['locus_tag'][0]
        elif 'db_xref' in featureTagDictKeys:
            geneName = self.tagDict["db_xref"][0]
        else:
            geneName = ''
...
class SudokuGridEntrySummaryLine:
    ...
    def printSummaryLineForDirectCSVOutput(self, gridReportingChoice='sudokuGrid', \
                                           gridReportingOn=False, printHeaderLine=False,
                                           minimumPickMode='inverseFeatureCount', \
...
def ImportFeatureArrayFromGenBank(genBankFile):
    ...
    featureNameRe = re.compile(r'\s+/db_xref="(.+)"')
