...
# Modification of WriteSudokuGridSummary that allows output of feature names as well
	...
	+ 'Fractional Distances of Transposon from Translation Starts,Locus Tags,Voigt Score\n'
			...
			for rowPool in rowPools:
				for colPool in colPools:
					...
					while i < len(readAlignmentCoords):
						...
						voigtScoreStrings.append(str(coord.locatabilityScore))
					...
					voigtScoreStr = '"' + ExportListForXML(voigtScoreStrings, delimeter=', ') + '"'
					
					outputStr += plateName + ',' + prPool + ',' + pcPool +  ',' + rowPool \
					+ colPool + ',' + coordsStr + ',' \
					+ locatabilitiesStr + ',' + featureNameStrings + ',' + distsStr + ',' \
					+ fracDistsStr + ',' + locusTagStr + ',' + voigtScoreStr + '\n'
