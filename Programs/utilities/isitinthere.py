from utilities.grid import SudokuWell, SudokuGenomicCoord
...
# ------------------------------------------------------------------------------------------------ #
def ReverseReducePoolPresenceTable(poolPresenceTable, uniqueCoordsPutInExpanded):
	import numpy
	reducedPoolPresenceTable = []
	linesToDelete = []
	i = 0
	while i < len(poolPresenceTable):
		coord = poolPresenceTable[i]['readAlignmentCoord']
		if coord in uniqueCoordsPutInExpanded:
			linesToDelete.append(i)
		i += 1
...
rowPools, colPools, prPools, pcPools, threshold):
	...
	import pdb
	from utilities.pool import FindAddressCoords2
...
def PopulateColonyPurifiedSudokuGrid(sudokuGridLookupDict, poolPresenceTable, rowPools, colPools, \
	...
	import pdb
	from utilities.grid import SudokuGenomicCoord
...
# Modification of WriteSudokuGridSummary that allows output of feature names as well
	import pdb
	from utilities.xml import ExportListForXML
...
#	pdb.set_trace()
	...
	for key in cpKeys:
		...
		for prPool in prPools: #SEANCOMMENT: Don't we already know the PR/PC, why do we loop through them?
			for pcPool in pcPools:
...
rowPoolKey='rowPool', colPoolKey='colPool'):
	...
	from utilities.utils import ConvertWellIDToRowAndColumn
	import pdb
#	pdb.set_trace()
...
def GeneratePoolPresenceDict(poolPresenceTable, addressPools):
	from utilities.pool import FindAddressCoords
	i = 0
	poolPresenceDict = {}
	while i < len(poolPresenceTable):
		coord = poolPresenceTable[i]['readAlignmentCoord']
...
def ReviewPastData(filename):

	import pdb
	from utilities.utils import ParseCSVLine
	...
	while i < len(data):
		...
		if line[1] != 'Q':
			...
			if int(columns[9].split(',')[0]) > 0:
				for val in columns[9].split(','):
					transposons.append(int(val))
			elif columns[4] != 'NA':
				well_plates.append((columns[1],columns[2],columns[3]))
				plate_mapping[(columns[1],columns[2],columns[3])] = columns[0]

		i += 1
	return well_plates, transposons, plate_mapping


def DocumentUnexpectedFinds(sudokuGridLookupDict, poolPresenceTable, rowPools, colPools, \
	prPools, pcPools, readCountThreshold, well_plates, featureArray):
	from utilities.grid import SudokuGenomicCoord

	i = 0
	unexpectedFinds = {}
	for well_plate in well_plates:
		unexpectedFinds[well_plate] = []
	while i < len(poolPresenceTable):
		...
		for address in possibleAddresses:
			[row, col, pr, pc] = address[0].split('_')
			if (pr, pc, row + col) in unexpectedFinds:
				readCount = address[1]
				newCoord = SudokuGenomicCoord(coord, 'direct', 0.0, readCount)
				unexpectedFinds[(pr,pc,row+col)].append(UpdateCoordinateFeatures(newCoord, featureArray))

		i += 1

	return unexpectedFinds


def UpdateCoordinateFeatures(readAlignmentCoord, featureArray):
	coord = readAlignmentCoord.coord
	...
	if lenDist == 0:
		readAlignmentCoord.distanceFromFeatureTranslationStart.append('NA')
	return readAlignmentCoord


def WriteUnexpectedFindsOutputTable(filename, unexpected_finds, plate_mapping):
	from utilities.xml import ExportListForXML
	...
	for well in unexpected_finds:
		...
		cpPlate = plate_mapping[well]
		cpWell = well[2]
		cpPlateRow = well[0]
		cpPlateCol = well[1]

		if len(unexpected_finds[well]) > 0:

			likelyCoords = unexpected_finds[well]
	...
	headerStr = 'QC Plate Row,QC Plate Column,QC Well,' \
				+ 'Confirmed Predicted Transposon Coordinates,' \
				+ 'Features Disrupted,' \
				+ 'Distances of Transposon from Translation Starts,' \
				+ 'Fractional Distances of Transposon from Translation Starts,Locus Tags\n'
